...
let leftShoulder, circleMask, angleOnCircle, myAvatarObj;

let videoOptions, preferredCam;


function setup() {
    myCanvas = createCanvas(712, 712);
    circleMask = createGraphics(712, 712);
    myCanvas.hide();
    ...
    leftShoulder = { "x": myVideo.width / 2, "y": myVideo.height / 2 };
...
function gotPoses(results) {
    ...
    poses = results;
    progress = "predicting";
    let thisShoulder = results[0].pose.leftShoulder;
    ...
    if (thisShoulder.confidence > .8) {
      leftShoulder.x = thisShoulder.x;
      leftShoulder.y = thisShoulder.y;

      let xDiff = poses[0].pose.rightShoulder.x - poses[0].pose.leftShoulder.x;
      let yDiff = poses[0].pose.rightShoulder.y - poses[0].pose.leftShoulder.y;
...
function draw() {
    ...
    circleMask.fill(0, 0, 0, 2);//set alpha of mask
    circleMask.noStroke();
    //circleMask.ellipse(nose.x, nose.y, 48, 48)//use nose pos to draw alpha
    circleMask.ellipse(leftShoulder.x, leftShoulder.y, 60, 60)//use nose pos to draw alpha
...
function init3D() {
    ...
    var videoGeometry = new THREE.PlaneGeometry(712, 712);
