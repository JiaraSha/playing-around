...
let leftElbow, circleMask, angleOnCircle, myAvatarObj;
...
function setup() {
    ...
    leftElbow = { "x": myVideo.width / 2, "y": myVideo.height / 2 };
...
function gotPoses(results) {
    ...
    poses = results;
    progress = "predicting";
    let thisElbow = results[0].pose.leftElbow;
    ...
    if (thisElbow.confidence > .8) {
      leftElbow.x = thisElbow.x;
      leftElbow.y = thisElbow.y;

      let xDiff = poses[0].pose.rightElbow.x - poses[0].pose.leftElbow.x;
      let yDiff = poses[0].pose.rightElbow.y - poses[0].pose.leftElbow.y;
...
function draw() {
    ...
    circleMask.ellipse(leftElbow.x, leftElbow.y, 60, 60)//use nose pos to draw alpha
