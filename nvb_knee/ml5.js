...
let rightKnee, circleMask, angleOnCircle, myAvatarObj;
...
function setup() {
    ...
    rightKnee = { "x": myVideo.width / 2, "y": myVideo.height / 2 };
...
function gotPoses(results) {
    ...
    poses = results;
    progress = "predicting";
    let thisKnee = results[0].pose.rightKnee;
    ...
    if (thisKnee.confidence > .8) {
      rightKnee.x = thisKnee.x;
      rightKnee.y = thisKnee.y;

      let xDiff = poses[0].pose.leftKnee.x - poses[0].pose.rightKnee.x;
      let yDiff = poses[0].pose.leftKnee.y - poses[0].pose.rightKnee.y;
...
function draw() {
    ...
    circleMask.fill(0, 0, 0, 12);//set alpha of mask
    circleMask.noStroke();
    //circleMask.ellipse(nose.x, nose.y, 60, 60)//use nose pos to draw alpha
    circleMask.ellipse(rightKnee.x, rightKnee.y, 60, 60)//use nose pos to draw alpha
