...
let rightWrist, circleMask, angleOnCircle, myAvatarObj;
...
function setup() {
    ...
    rightWrist = { "x": myVideo.width / 2, "y": myVideo.height / 2 };
...
function gotPoses(results) {
    ...
    if (thisWrist.confidence > .8) {
      rightWrist.x = thisWrist.x;
      rightWrist.y = thisWrist.y;

      let xDiff = poses[0].pose.leftWrist.x - poses[0].pose.rightWrist.x;
      let yDiff = poses[0].pose.leftWrist.y - poses[0].pose.rightWrist.y;
...
function draw() {
    ...
    circleMask.ellipse(rightWrist.x, rightWrist.y, 60, 60)//use nose pos to draw alpha
