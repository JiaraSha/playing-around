...
let leftAnkle, circleMask, angleOnCircle, myAvatarObj;
...
function setup() {
    ...
    leftAnkle = { "x": myVideo.width / 2, "y": myVideo.height / 2 };
...
function gotPoses(results) {
    ...
    poses = results;
    progress = "predicting";
    let thisAnkle = results[0].pose.leftAnkle;
    ...
    if (thisAnkle.confidence > .8) {
      leftAnkle.x = thisAnkle.x;
      leftAnkle.y = thisAnkle.y;

      let xDiff = poses[0].pose.leftAnkle.x - poses[0].pose.rightAnkle.x;
      let yDiff = poses[0].pose.leftAnkle.y - poses[0].pose.rightAnkle.y;
...
function draw() {
    ...
    circleMask.fill(0, 0, 0, 6);//set alpha of mask
    circleMask.noStroke();
    //circleMask.ellipse(nose.x, nose.y, 60, 60)//use nose pos to draw alpha
    circleMask.ellipse(leftAnkle.x, leftAnkle.y, 48, 48)//use nose pos to draw alpha
