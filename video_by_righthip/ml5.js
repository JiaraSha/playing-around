...
let rightHip, circleMask, angleOnCircle, myAvatarObj;
...
function setup() {
    ...
    rightHip = { "x": myVideo.width / 2, "y": myVideo.height / 2 };
...
function gotPoses(results) {
    ...
    poses = results;
    progress = "predicting";
    let thisHip = results[0].pose.rightHip;
    ...
    if (thisHip.confidence > .8) {
      rightHip.x = thisHip.x;
      rightHip.y = thisHip.y;

      let xDiff = poses[0].pose.leftHip.x - poses[0].pose.rightHip.x;
      let yDiff = poses[0].pose.leftHip.y - poses[0].pose.rightHip.y;
...
function draw() {
    ...
    circleMask.ellipse(rightHip.x, rightHip.y, 60, 60)//use nose pos to draw alpha
