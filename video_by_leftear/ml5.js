...
let leftEar, circleMask, angleOnCircle, myAvatarObj;
...
function setup() {
    ...
    leftEar = { "x": myVideo.width / 2, "y": myVideo.height / 2 };
...
function gotPoses(results) {
    ...
    poses = results;
    progress = "predicting";
    let thisEar = results[0].pose.leftEar;
    ...
    if (thisEar.confidence > .8) {
      leftEar.x = thisEar.x;
      leftEar.y = thisEar.y;

      let xDiff = poses[0].pose.rightEar.x - poses[0].pose.leftEar.x;
      let yDiff = poses[0].pose.rightEar.y - poses[0].pose.leftEar.y;
...
function draw() {
    ...
    circleMask.ellipse(leftEar.x, leftEar.y, 60, 60)//use nose pos to draw alpha
